% function LPQFeature = lpq(img,blocksize)
% %apply lpq_ on the grid on img
% [m,n,c] = size(img);
% if c == 3
%     img = rgb2gray(img);
% end
% LPQFeatures = zeros(m*n/blocksize^2,256);
% count = 0;
% for p = 1:(m/blocksize)
%     for q = 1:(n/blocksize)
%         count = count +1;
%         %mask = [1+p*blocksize:blocksize+p*blocksize,1+q*blocksize:blocksize+q*blocksize];
%         LPQFeatures(count,:) = ...
%             lpq_(img(1+(p-1)*blocksize:blocksize+(p-1)*blocksize,1+(q-1)*blocksize:blocksize+(q-1)*blocksize),3,0,1,'nh');
%     end
% end
% LPQFeature = reshape(LPQFeatures',[1,m*n/blocksize^2*256]);
